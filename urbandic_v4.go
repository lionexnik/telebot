...
func GetStockQuotes(symbols string, dataFormat ...int) (quotes string, err error) {
	...
	symbolsString = strings.Replace(symbols," ", "%20",-1)

	//handle the data format, defaults to json
	urlString = fmt.Sprintf("https://mashape-community-urban-dictionary.p.rapidapi.com/define?term=%s",symbolsString)
	//пример:
	...
	if len(string(buf)) > 20 {
		arr := strings.SplitAfter(string(buf), "permalink")
		//quotes= strings.Replace(string(arr[0]),"list","",-1)
		arr = strings.SplitAfter(string(arr[0]), `"definition"`)

		quotes = strings.TrimRight(string(arr[1]), `","permalink`)
		quotes = strings.TrimLeft(quotes, `:"`)
		quotes = `"` + quotes + `"`
